...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="/">
   ...
   <database id="{$db_id}" name='a-team' version="{$db_version}" />
    ...
    <xsl:if test="$top_partition_type = 'shelf'">
  ...
  <!--
  <xsl:template name="remove_para" match="//body//div[@class='Folder']/p[@class='REMOVE']" />
  -->
  <xsl:template name="remove_document_div_table" match="//body//div[@class='document']/table[@class='REMOVE']" />
  <xsl:template name="remove_Folder_div_table" match="//body//div[@class='Folder']/table[@class='REMOVE']" />
  ...
  <!-- Extract the folder -->
  <xsl:template name="folder" match="//body//div[@class='Folder']">
        <document revision="1" lang_id="{$lang_id}" type="shelf">
  ...
  <xsl:template name="document" match="//body//div[@class='Document']">
        <document revision="1" lang_id="{$lang_id}" type="{$doc_type}">
        <xsl:attribute name="id">
            <xsl:value-of select="@id"/>
        </xsl:attribute>
        <xsl:attribute name="group_id">
	...
	<xsl:if test="string-length(@target_type) &gt; 0">
		...
		<xsl:when test="@target_type = 'external'">
                    <xsl:attribute name="exdata_id">
                        <xsl:value-of select="@msgid"/>
                    </xsl:attribute>
		    <!-- Move this later to the objects element -->
		    <exdata>
                        <xsl:attribute name="id">
                            <xsl:value-of select="@msgid"/>
                        </xsl:attribute>
  ...
  <!-- Extract the section -->
  <xsl:template name="section" match="//body//div[@class='Section']">
	<section>
	<section_title>
            <xsl:value-of select="@title"/>
	</section_title>
  ...
  <xsl:template name="task" match="//body//div[@class='Task']">
	<task>
	<name>
            <xsl:value-of select="@title"/>
	</name>
  ...
  <xsl:template name="processParagraph" match="p">
    ...
    <xsl:choose>
	    <xsl:when test="starts-with(@class,'REMOVE')">
	    </xsl:when>

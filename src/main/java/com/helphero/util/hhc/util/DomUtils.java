...
public abstract class DomUtils {
	static Logger logger = Logger.getLogger(DomUtils.class);
	  /**
	  ...
	  public static void replaceNode(Node newNode, Node oldNode) {
	    ...
	    if(parentNode == null) {
	      logger.error("Cannot replace node [" + oldNode + "] with [" + newNode + "]. [" + oldNode + "] has no parent.");
	    } else {
	  ...
	  public static void replaceNode(NodeList newNodes, Node oldNode, boolean clone) {
	    ...
	    Node parentNode = oldNode.getParentNode();
	        ...
	        if(parentNode == null) {
	          logger.error("Cannot replace [" + oldNode + "] with a NodeList. [" + oldNode + "] has no parent.");
	      return;
	    }
	    ...
	    List nodeList = DomUtils.copyNodeList(newNodes);
	      ...
	      if(!elements.isEmpty()) {
	        logger.error("Request to replace the Document root node with a 1+ in length NodeList.  Replacing root node with the first element node from the NodeList.");
	        parentNode.removeChild(oldNode);
	        parentNode.appendChild((Node)elements.get(0));
	      } else {
	        logger.error("Cannot replace document root element with a NodeList that doesn't contain an element node.");
	      }
	        } else {
	    ...
	    public static void insertBefore(Node newNode, Node refNode) {
	      ...
	      if(parentNode == null) {
	        logger.error("Cannot insert [" + newNode + "] before [" + refNode + "]. [" + refNode + "] has no parent.");
	      return;
	    }
	        
	        if(parentNode instanceof Document && newNode.getNodeType() == Node.ELEMENT_NODE) {
	        	logger.error("Request to insert an element before the Document root node.  This is not allowed.  Replacing the Document root with the new Node.");
	  ...
	  public static void insertBefore(NodeList newNodes, Node refNode) {
	    ...
	    if(parentNode == null) {
	      logger.error("Cannot insert a NodeList before [" + refNode + "]. [" + refNode + "] has no parent.");
	      return;
	    }
	    ...
	    List nodeList = DomUtils.copyNodeList(newNodes);
	      ...
	      if(!elements.isEmpty()) {
	        logger.error("Request to insert a NodeList before the Document root node.  Will replace the root element with the 1st element node from the NodeList.");
	              parentNode.removeChild(refNode);
	              parentNode.appendChild((Node)elements.get(0));
	      } else {
	        logger.error("Cannot insert beforen the document root element from a NodeList that doesn't contain an element node.");
	      }
	  ...
	  public static void removeElement(Element element, boolean keepChildren) {
	    ...
	    if(parent == null) {
	      logger.error("Cannot remove element [" + element + "]. [" + element + "] has no parent.");
	      return;
	    }
	    ...
	    if (parent instanceof Document) {
	      ...
	      if(!keepChildren) {
	        logger.error("Cannot remove document root element [" + DomUtils.getName(element) + "] without keeping child content.");
	      } else {
	        ...
	        if(childElements != null && !childElements.isEmpty()) {
	          ...
	          logger.error("Cannot remove empty document root element [" + DomUtils.getName(element) + "].");
	        }
	      }
	    } else {
	  ...
	  public static Node getPreviousSibling(Node node, short nodeType) {
	    Node parent = node.getParentNode();
	        if(parent == null) {
	          logger.error("Cannot get node [" + node + "] previous sibling. [" + node + "] has no parent.");
	      return null;
	    }
	  ...
	  public static int countNodesBefore(Node node, short nodeType) {
	    Node parent = node.getParentNode();
	        if(parent == null) {
	          logger.error("Cannot count nodes before [" + node + "]. [" + node + "] has no parent.");
	      return 0;
	    }
	  ...
	  public static int countNodesBetween(Node node1, Node node2, short nodeType) {
	    Node parent1 = node1.getParentNode();
	        if(parent1 == null) {
	          logger.error("Cannot count nodes between [" + node1 + "] and [" + node2 + "]. [" + node1 + "] has no parent.");
	      return 0;
	    }
	    
	    Node parent2 = node2.getParentNode();
	        if(parent2 == null) {
	          logger.error("Cannot count nodes between [" + node1 + "] and [" + node2 + "]. [" + node2 + "] has no parent.");
	      return 0;
	    }
	  ...
	  public static int countNodesBetween(Node node1, Node node2) {
	    ...
	    if(parent1 != parent2) {
	      logger.error("Cannot count nodes between [" + node1 + "] and [" + node2 + "]. These nodes do not share the same sparent.");
	      return 0;
	    }
	  ...
	  public static String getTextBefore(Node node) {
	    Node parent = node.getParentNode();
	    if(parent == null) {
	      logger.error("Cannot get text before node [" + node + "]. [" + node + "] has no parent.");
	      return "";
	    }
	  ...
	  public static String getTextBetween(Node node1, Node node2) {
	    Node parent1 = node1.getParentNode();
	        if(parent1 == null) {
	      logger.error("Cannot get text between nodes [" + node1 + "] and [" + node2 + "]. [" + node1 + "] has no parent.");
	      return "";
	    }
	    
	    Node parent2 = node2.getParentNode();
	        if(parent2 == null) {
	        	logger.error("Cannot get text between nodes [" + node1 + "] and [" + node2 + "]. [" + node2 + "] has no parent.");
	      return "";
	    }
	    
	    if(parent1 != parent2) {
	    	logger.error("Cannot get text between nodes [" + node1 + "] and [" + node2 + "]. These nodes do not share the same sparent.");
	      return "";
	    }
	 ...
	 /**
	  * Convenience method to extract and concatenate all child element text
	  *  
	  * @param xNode - Node
	  * @return String Concatenated child elements text
	  */
	public static String extractTextChildren(Node xNode) {
